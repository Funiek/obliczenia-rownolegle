...
i8* sobel_operator(i8* pixels, int width, int height, int start, int end, int rank) {
...
i8* sobel_normalize(i8* pixels, int start, int end, int rank) {
    i8* new_pixels = (i8*)malloc((end-start)*sizeof(i8));
    i8 max = pixels[0];
    i8 min = pixels[0];
    ...
    for(int i = 0; i < end - start; i++) {
        new_pixels[i] = (i8)(((double)(pixels[i] - min) / (double)(max - min)) * 255.0); // wartość znormalizowana
    }

    return new_pixels;
}

i8* median(i8* pixels, int width, int height, int start, int end, int rank) {
    i8* local_pixels = (i8*)malloc((end-start)*sizeof(i8));
